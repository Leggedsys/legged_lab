...
DOG_URDF_CFG = ArticulationCfg(
    ...
    # NOTE: 左右 hip 关节在 URDF 中 axis 方向相反（右侧 FR/RR axis="-1 0 0", 左侧 FL/RL axis="1 0 0"），
    # 但 URDF→USD 转换时 axis 的符号信息丢失，导致 USD 中所有 hip 的 axis 都变成正方向。
    # 因此只有右侧（FR/RR）hip 的值需要取反，才能在仿真中得到正确的向外摆腿姿态。
    # 左侧（FL/RL）axis 不变，无需取反。
    # 此值与 URDF 的约定不一致，是 Isaac Lab 侧的适配。部署到真实机器人时需反向映射。
    init_state=ArticulationCfg.InitialStateCfg(
        pos=(0.0, 0.0, 0.28),
        joint_pos={
            # hip（外摆为正；右侧 FR/RR 因 USD axis 翻转取反，左侧 FL/RL 不变）
            "^FL_hip_joint$":    0.1,
            "^FR_hip_joint$":   -0.1,
            "^RL_hip_joint$":    0.1,
            "^RR_hip_joint$":   -0.1,
...
DOG_JOINT_SIGN = {
    "FL_hip_joint":   1.0,   # 左侧，USD axis 不变
    "FR_hip_joint":  -1.0,   # 右侧，USD axis 翻转
    "RL_hip_joint":   1.0,   # 左侧，USD axis 不变
    "RR_hip_joint":  -1.0,   # 右侧，USD axis 翻转
    "FL_thigh_joint": 1.0,
    "FR_thigh_joint": 1.0,
    "RL_thigh_joint": 1.0,
    "RR_thigh_joint": 1.0,
